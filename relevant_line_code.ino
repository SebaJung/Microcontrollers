...
void loop() {
  ...
  */
  ...
  if (centerSensor >= 200 && (rightSensor < 200) && (leftSensor < 200) && (distance <= 950)


      do                        // this set of statements refer to fast pace
        goForward();
        while (centerSensor >= 200 && (rightSensor < 200) && (leftSensor < 200) && (distance <= 950));

          // These are not mutually exclusive so I made it 3 'if' statements

      if (leftSensor >= 200) {
        leftNoDetect();           // Increase speed of left wheel(or reduce speed of right wheel)
          // to encourage the car to yaw right OCR0B = 200 or OCR2B = 100

        }
  if (rightSensor >= 200) {
  rightNoDetect();                  // Increase speed of right wheel(or reduce speed of left wheel)
    //to encourage the car to yaw left OCR2B = 200 or OCR0B = 100
  }

  if (leftSensor >= 200) {
  leftNoDetectSlow();// Increase speed of left wheel(or reduce speed of right wheel) to encourage the car to yaw right
    //OCR0B = 200 or OCR2B = 100
  }
  if (rightSensor >= 200) {
  rightNoDetectSlow();                  // Increase speed of right wheel(or reduce speed of left wheel)
    //to encourage the car to yaw left OCR2B = 200 or OCR0B = 100
  }
...
void leftNoDetect() {         // if left sensor off tape...
...
void rightNoDetect() {          // if left bumper is hit...
