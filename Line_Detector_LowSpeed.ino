...
void loop() {
  ...
  if ((centerSensor >= 200)) {
    if ((centerSensor >= 200) && (leftSensor < 100) && (rightSensor < 100)) {
    goForward();
  }
    if (leftSensor >= 200)
  ...
  if ((distance > 2000) && (centerSensor < 100) && (leftSensor < 100) && (rightSensor < 100)) {
    OCR0B = 0;
    OCR2B = 0;
  }
...
void goForward() {                  // if nothing is hit...
  OCR0A = 0;
  OCR0B = 120;                      // sends the left motor forward
  OCR2A = 0;
  OCR2B = 120;                      // sends the right motor forward
}
